...
const viewSelect = resetNodeById('viewSelect');
...
function applyView(value) {
  const is3D = value === '3d';
  document.body.classList.toggle('show-bom', !is3D);

  if (is3D) {
    document.body.classList.remove('show-bom');
    canvas.style.display = 'block';
    if (bomPageEl) bomPageEl.style.display = 'none';
  } else {
    document.body.classList.add('show-bom');
    canvas.style.display = 'none';
  }

  if (bomWrapEl) bomWrapEl.style.display = value === 'walls-bom' ? '' : 'none';
  if (bomPageEl) bomPageEl.style.display = value === 'base-bom' ? 'block' : 'none';
...
if (viewSelect && !viewSelect._wired) {
  viewSelect._wired = true;
  viewSelect.addEventListener('change', () => {
    if (viewSelect.value === '3d') {
      document.body.classList.remove('show-bom');
      canvas.style.display = 'block';
    } else {
      document.body.classList.add('show-bom');
...
/* Keep addEventListener wiring (fallback): only toggles if inline onclick didn't */
if (controlsToggle && !controlsToggle._wired) {
  controlsToggle._wired = true;
  controlsToggle.addEventListener('click', () => {
    ...
    setTimeout(() => {
      if (panel.style.display !== before) return; // inline already toggled
      const willHide = (panel.style.display !== 'none');
      panel.style.display = willHide ? 'none' : '';
      btn.textContent = willHide ? 'Show Inputs' : 'Hide Inputs';
    }, 0);
  });
}
