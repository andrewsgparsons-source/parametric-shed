...
function applyViewByBodyClass(value) {
  const is3D = value === '3d';

  document.body.classList.toggle('mode-3d', is3D);
  document.body.classList.toggle('show-bom', !is3D);
  document.body.classList.toggle('show-walls', value === 'walls-bom');
  ...
  if (bomWrapEl) {
    bomWrapEl.style.display = value === 'walls-bom' ? '' : 'none';
  }
}

function render(state) {
  applyViewByBodyClass(viewSelect.value);
