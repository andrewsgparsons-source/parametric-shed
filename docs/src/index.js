...
// UI elements (reset to remove any pre-existing listeners from prior builds)
const viewSelect = resetNodeById('viewSelect');
const controlsToggle = resetNodeById('controlsToggle');
...
function setUiHidden(hidden) {
  ...
  if (controlsToggle) controlsToggle.textContent = hidden ? 'Show Inputs' : 'Hide Inputs';
}

function applyView(value) {
  const is3D = value === '3d';
  const isWalls = value === 'walls-bom';
  const isBase = value === 'base-bom';
  ...
  if (is3D) {
    // Always ensure BOM containers are hidden in 3D
    bomWrapEl.style.display = 'none';
    bomPageEl.style.display = 'none';
  }
}
...
// Hide/Show Inputs wiring (mobile-safe)
if (controlsToggle && !controlsToggle._wired) {
  controlsToggle._wired = true;
  controlsToggle.addEventListener('click', () => {
    const hidden = document.body.classList.contains('ui-hidden') || (controls && controls.style.display === 'none');
    setUiHidden(!hidden);
  });
}
